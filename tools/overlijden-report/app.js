/**
 * EMS Overlijdensrapport Generator
 * ----------------------------------------
...
const STORAGE_KEY = "ems_tool_overlijden_report_v1";

const form = document.getElementById("reportForm");
const output = document.getElementById("reportOutput");
const costDisplay = document.getElementById("costDisplay");
const statusBox = document.getElementById("statusBox");
const deathTypeDisplay = document.getElementById("deathTypeDisplay");
const bodyDispositionDisplay = document.getElementById("bodyDispositionDisplay");
...
function updateMeta() {
  deathTypeDisplay.textContent = getValue("deathType") || "—";
  bodyDispositionDisplay.textContent = getValue("bodyDisposition") || "—";
}
...
function generateReport() {
  ...
  const report = `
=== ${appConfig.reportType || "OVERLIJDENSRAPPORT"} ===

[ALGEMENE BASISGEGEVENS]
${line("Procedure / dossier", getValue("procedureType"))}
${line("Naam overledene", getValue("patientName"))}
${line("Patiënt-ID", getValue("patientId"))}
${line("Datum & tijd rapport", formatDateTime(getValue("dateTime")))}
${line("Locatie", getValue("location"))}
${line("Rapporteur / vaststellend arts", getValue("staffLead"))}
${line("Assistent(en) / aanwezigen", getValue("staffAssist"))}

[CONTEXT EN VASTSTELLING]
${line("Context van aantreffen / overlijden", getValue("deceasedContext"))}
${line("Tijdstip overlijden / vaststelling", getValue("deceasedTime"))}
${line("Plaats van overlijden", getValue("deceasedPlace"))}
${line("Type overlijden", getValue("deathType"))}
${line("Forensische prioriteit", getValue("forensicPriority"))}
${line("Vaststellingen bij onderzoek", joinList(getChecked("deathExamFlags")))}
${line("Vaststellingen bij onderzoek — detail", getValue("deathExam"))}
${line("Reanimatie / voorafgaande interventies", getValue("resuscitationStatus"))}
${line("Vermoedelijke doodsoorzaak", getValue("presumedCause"))}

[AFHANDELING EN MELDINGEN]
${line("Forensische / administratieve stappen", joinList(getChecked("forensicHandlingFlags")))}
${line("Meldingen / communicatie", joinList(getChecked("notificationFlags")))}
${line("Forensische / administratieve afhandeling", getValue("forensicHandling"))}
${line("Melding familie / bevoegde diensten", getValue("nextOfKin"))}
${line("Bestemming lichaam / overdracht", getValue("bodyDisposition"))}
${line("Follow-up / vervolgactie", getValue("followUp"))}
${line("Persoonlijke bezittingen / registraties", getValue("propertyNotes"))}
...
function getAllFormData() {
  ...
  result.deathExamFlags = getChecked("deathExamFlags");
  result.forensicHandlingFlags = getChecked("forensicHandlingFlags");
  result.notificationFlags = getChecked("notificationFlags");
...
function restoreFromLocalStorage() {
  try {
    ...
    Object.entries(data).forEach(([key, value]) => {
      if (
        key === "deathExamFlags" ||
        key === "forensicHandlingFlags" ||
        key === "notificationFlags" ||
        key === "supplements"
      ) return;
    ...
    if (Array.isArray(data.deathExamFlags)) {
      document.querySelectorAll('input[name="deathExamFlags"]').forEach((checkbox) => {
        checkbox.checked = data.deathExamFlags.includes(checkbox.value);
      });
    }

    if (Array.isArray(data.forensicHandlingFlags)) {
      document.querySelectorAll('input[name="forensicHandlingFlags"]').forEach((checkbox) => {
        checkbox.checked = data.forensicHandlingFlags.includes(checkbox.value);
      });
    }

    if (Array.isArray(data.notificationFlags)) {
      document.querySelectorAll('input[name="notificationFlags"]').forEach((checkbox) => {
        checkbox.checked = data.notificationFlags.includes(checkbox.value);
      });
    }
...
function downloadTxt() {
  ...
  const filename = `overlijdensrapport-${buildFileTimestamp()}.txt`;
...
function downloadJson() {
  ...
  const payload = {
    type: appConfig.reportType || "Overlijdensrapport",
  ...
  const filename = `overlijdensrapport-${buildFileTimestamp()}.json`;
...
async function initApp() {
  try {
    ...
    populateSelect("deathType", appConfig.deathTypeOptions || []);
    populateSelect("forensicPriority", appConfig.forensicPriorityOptions || []);
    populateSelect("bodyDisposition", appConfig.bodyDispositionOptions || []);
    populateCheckboxGroup("deathExamFlagsContainer", "deathExamFlags", appConfig.deathExamFlags || []);
    populateCheckboxGroup("forensicHandlingFlagsContainer", "forensicHandlingFlags", appConfig.forensicHandlingFlags || []);
    populateCheckboxGroup("notificationFlagsContainer", "notificationFlags", appConfig.notificationFlags || []);
